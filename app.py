        ...
        try:
            ...
            if len(all_values) > 1:
                ...
                if '발생량' in df.columns:
                    df['발gen량'] = df['발생량'].astype(str).str.replace(',', '')
                    df['발생량'] = pd.to_numeric(df['발gen량'], errors='coerce').fillna(0)
...
if login():
    ...
    # 🎨 로그인 후 메인 대시보드 타이틀 및 우측 상단 'By In02' 배치
    st.markdown(
        """
    ...
    with tab2:
        ...
        with st.container():
            # 폐기 일자 및 조 선택 (A, B, C, D, E)
            p_col1, p_col2 = st.columns(2)
            with p_col1:
                dispose_date = st.date_input("폐기 일자", datetime.now(), key="dispose_date_input")
            with p_col2:
                dispose_shift = st.selectbox("조 선택", ["A조", "B조", "C조", "D조", "E조"], key="dispose_shift_input")
            
            st.markdown("---")
            
            # 폐기구분 라디오 버튼 선택
            st.write("**폐기구분**")
            dispose_type = st.radio("폐기 분류 유형 선택", ["정규폐기 (주간)", "마대폐기 (선별, 청소 등)"], horizontal=True, key="dispose_type_input")
            
            st.markdown("---")
            
            # 실제 스크랩 실중량 입력
            st.write("**실제 처리 중량 입력**")
            dispose_weight = st.number_input("폐기 및 처리 중량 (kg)", min_value=0.0, step=0.5, key="dispose_weight_input", placeholder="실측 처리 중량을 입력하세요.")
            
            st.markdown("---")
            
            # 기타 참고사항
            st.write("**기타 참고사항**")
            dispose_memo = st.text_input("비고 사항 입력", placeholder="특이사항이나 메모를 입력하세요.", key="dispose_memo_input")
                ...
                if dispose_weight <= 0:
                    ...
                    if client:
                        try:
                            ...
                            # 데이터 순서 최적화하여 누적
                            proc_data = [
                                str(dispose_date),      # A열: 폐기 일자
                                dispose_shift,          # B열: 조 선택 (A~E)
                                dispose_type,           # C열: 폐기 구분
                                dispose_weight,         # D열: 처리 중량
                                dispose_memo,           # E열: 비고 사항
                                datetime.now().strftime("%Y-%m-%d %H:%M:%S"), # F열: 등록 시간
                                st.session_state["user_id"] # G열: 등록자 ID
                            ]
                            proc_sheet.append_row(proc_data)
                            
                            st.session_state["disp_success_msg"] = f"✅ [{dispose_shift} / {dispose_type}] 실제 처리 중량 {dispose_weight:,} kg 기록이 어긋남 없이 안전하게 저장되었습니다!"
                            st.cache_resource.clear()
                            if "raw_df" in st.session_state: 
                                del st.session_state["raw_df"]
                            st.rerun()
                        except Exception as e:
                            st.error(f"구글 시트에 처리량 데이터를 쓰던 중 에러가 발생했습니다: {e}")
    ...
    with tab3:
        ...
        if not df.empty:
            ...
            with f_col1:
                selected_shifts = st.multiselect("생산 조 필터", options=["전체", "A조", "B조", "C조", "D조"], default=["전체"], key="sel_shift_filter")
            with f_col2:
                selected_macs = st.multiselect("발생 호기 필터", options=["전체"] + mac_options, default=["전체"], key="sel_mac_filter")

            filtered_df = df.copy()
            # 💡 [교정 파트] && ➔ and 로 수정 완료 (454줄, 456줄)
        ...
        if not df_analysis.empty:
            ...
            if not df_filtered.empty:
                ...
                if '년월' in df_filtered.columns and '생산 조' in df_filtered.columns:
                    ...
                    fig_monthly.update_traces(textposition='outside', textfont=dict(color='#0052CC', size=14, family='Pretendard, sans-serif', weight='bold'))
                    ...
                    if is_mobile_view:
                        ...
                        fig_monthly.update_yaxes(showgrid=False, zeroline=False, tickfont=dict(size=14, family='Pretendard, sans-serif', weight='bold'))
                    else:
                        fig_monthly.update_xaxes(showgrid=False, zeroline=False, tickfont=dict(size=14, family='Pretendard, sans-serif', weight='bold'))
                ...
                if '추정 원인' in df_filtered.columns:
                    ...
                    fig_cause.update_traces(marker_color='#4A90E2', textposition='outside', textfont=dict(color='#0052CC', size=14, family='Pretendard, sans-serif', weight='bold'))
                    ...
                    fig_cause.update_yaxes(showgrid=False, zeroline=False, tickfont=dict(size=14, family='Pretendard, sans-serif', weight='bold'))
                ...
                if target_mac_col:
                    ...
                    fig_mac.update_traces(marker_color='#7ED321', textposition='outside', textfont=dict(color='#0052CC', size=14, family='Pretendard, sans-serif', weight='bold'))
                    ...
                    fig_mac.update_xaxes(showgrid=False, zeroline=False, tickfont=dict(size=14, family='Pretendard, sans-serif', weight='bold'))
                ...
                if '처리 판단' in df_filtered.columns:
                    ...
                    fig_status.update_traces(marker_color='#F5A623', textposition='outside', textfont=dict(color='#0052CC', size=14, family='Pretendard, sans-serif', weight='bold'))
                    ...
                    fig_status.update_yaxes(showgrid=False, zeroline=False, tickfont=dict(size=14, family='Pretendard, sans-serif', weight='bold'))
