...
# --- 1. 페이지 설정 및 디자인 ---
st.set_page_config(page_title="정련Sub팀 스크랩 관리 시스템 v4.2", page_icon="⚙️", layout="wide")
...
if login():
    ...
    # --- [TAB 2] 실제 스크랩 처리량 등록 (요구사항 반영 버전) ---
    with tab2:
        ...
        with st.container():
            ...
            # 🌟 [요구사항 반영] 스크랩(고무) 선택 시 추가 정보 입력 영역 활성화
            rubber_shift = "N/A"
            rubber_comp = "N/A"
            ...
            if dispose_type == "스크랩(고무)":
                st.markdown("""
                    <div style='background-color: #eef2f7; padding: 12px; border-left: 5px solid #0052CC; border-radius: 4px; margin-bottom: 15px;'>
                        <strong style='color: #0052CC;'>🔍 스크랩(고무) 추가 정보 입력</strong>
                    </div>
                """, unsafe_allow_html=True)
                ...
                with rc2:
                    is_comp_unknown = st.checkbox("Comp'd 확인불가", key="comp_unknown_checkbox")
                    if is_comp_unknown:
                        rubber_comp = "확인불가"
                        # 비활성화된 시각적 피드백용 더미 UI
                        st.text_input("Comp'd명 (확인불가 선택됨)", value="확인불가", disabled=True)
                    else:
                        st.write("Comp'd명 입력")
                        ...
                        with cc3: r_comp_text = st.text_input("3자리 입력", max_chars=3, placeholder="예: R24", key="r_comp_text").upper().strip()
            ...
            if st.button("💾 처리 실적 저장하기", key="btn_dispose_submit"):
                # 유효성 검사 (스크랩 고무일 때 코드 미입력 차단)
                if dispose_type == "스크랩(고무)" and not is_comp_unknown and len(r_comp_text) < 3:
                    st.error("⚠️ Comp'd명 3자리를 정확히 입력하거나 '확인불가'를 체크해주세요.")
                    ...
                    if client:
                        try:
                            ...
                            # 기존 구조에 고무 정보가 있을 경우 메모에 조합하거나 열을 추가하여 기록 가능
                            # 여기서는 메모 앞부분에 [조/코드] 형태로 식별 가능하게 자동 결합해 데이터 유실을 방지합니다.
                            extended_memo = dispose_memo
                            if dispose_type == "스크랩(고무)":
                                extended_memo = f"[발생조:{rubber_shift} / Comp'd:{rubber_comp}] {dispose_memo}".strip()
