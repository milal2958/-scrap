        ...
        if not df_analysis.empty:
            ...
            if not df_mac_filtered.empty:
                ...
                fig_mac.update_layout(
                    height=450, 
                    bargap=0.2, # 세로 차트 격차 최적화
                    plot_bgcolor='rgba(0,0,0,0)',
                    paper_bgcolor='rgba(0,0,0,0)',
                    margin=dict(t=40, b=30, l=10, r=10), # 세로 차트 형태에 맞춤 상단 마진 확보
                    font=dict(family="Pretendard, Malgun Gothic, sans-serif", size=14),
                    hovermode="closest"
                )

                # 세로 차트는 y축 최대범위에 버퍼 여유 부여
            ...
            # ==========================================
            # 4️⃣ [신규 추가: 처리 판단별 발생량 분석 - 가로형 막대]
            # ==========================================
            ...
            if not df_status_filtered.empty:
                ...
                # 🛠️ 요구 조건에 따른 가로형(orientation='h') 막대그래프 셋업
                fig_status = px.bar(
                    status_summary,
                    x='발생량_ton',
                    y='처리 판단',
                ...
                fig_status.update_traces(
                    marker_color='#F5A623', # 구분감 높은 오렌지/황색 계열 마커 매칭
                    textposition='outside',
                    textfont=dict(color='#0052CC', size=18, family='Pretendard, Malgun Gothic, sans-serif', weight='bold')
                )
                ...
                fig_status.update_layout(
                    ...
                    plot_bgcolor='rgba(0,0,0,0)',
                    paper_bgcolor='rgba(0,0,0,0)',
                    margin=dict(t=20, b=20, l=10, r=140), # 수치 누락 차단을 위한 여백 확보
                    font=dict(family="Pretendard, Malgun Gothic, sans-serif", size=14),
                    hovermode="closest"
                )
