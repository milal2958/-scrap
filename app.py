...
# --- 1. 페이지 설정 및 디자인 ---
st.set_page_config(page_title="정련Sub팀 스크랩 관리 시스템 v3.3", page_icon="⚙️", layout="wide")
...
if login():
    ...
    # --- [TAB 3] 발생 정밀분석 (차트 업그레이드) ---
    with tab3:
        st.subheader("📊 스크랩 발생 데이터 시각화")
        ...
        if not df_analysis.empty:
            ...
            fig_monthly = px.bar(
                ...
                color_discrete_sequence=px.colors.qualitative.Bold # 세련된 다크톤 컬러셋
            )
            ...
            with col_b:
                if not df_filtered.empty:
                    ...
                    m_col1.metric("총 발생량", f"{total_kg:,.1f} kg", delta="핵심 지표")
                    
                    uncomp = df_filtered[df_filtered['처리 완료'] == False]
                    m_col2.metric("미완료 건수", f"{len(uncomp)} 건", delta=f"{uncomp['발생량_kg'].sum():,.1f} kg", delta_color="inverse")
