...
# --- 1. 페이지 설정 및 디자인 ---
st.set_page_config(page_title="정련Sub팀 스크랩 관리 시스템 v4.0", page_icon="⚙️", layout="wide")
...
if login():
    ...
    with tab1:
        ...
        with st.container():
            ...
            with comp_col3: comp_text = st.text_input("나머지 3글자 입력", max_chars=3, placeholder="예: R24", key="comp_text_input").upper()
    ...
    with tab2:
        ...
        with st.container():
            ...
            with p_col2: dispose_shift = st.selectbox("조 선택", ["A조", "B조", "C조", "D조", "E조"], key="dispose_shift_input")
            
            st.markdown("---")
            st.write("**폐기구분**")
            # 💡 변경 반영: 요구사항에 부합하는 5종 세분화 카테고리 적용
    ...
    with tab3:
        ...
        if not df.empty:
            ...
            try:
                ...
                # 🌟 [스마트 타겟 업데이트 구조 개편] API 과부하 및 속도 저하 차단
    ...
    with tab4:
        ...
        if not df_analysis.empty:
            df_analysis['발생량_ton'] = df_analysis.apply(
                lambda x: (x['발생량'] * 200) / 1000 if 'batch' in str(x['단위']).lower().strip() else x['발생량'] / 1000, axis=1
            )
            ...
            with ana_col1: start_date = st.date_input("분석 시작일", df_analysis['날짜'].min() if len(df_analysis)>0 else datetime.now().replace(day=1), key="ana_start")
            with ana_col2: end_date = st.date_input("분석 종료일", datetime.now(), key="ana_end")
