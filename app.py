        ...
        if not df_analysis.empty:
            ...
            if not df_trend_filtered.empty:
                ...
                if is_mobile_view:
                    ...
                    # 🔴 조별 합계 숫자 크기 확대 (size=18) 및 고딕체 지정
                    fig_monthly.update_traces(
                        textposition='outside',
                        textfont=dict(color='red', size=18, family='Pretendard, Malgun Gothic, sans-serif', weight='bold')
                    )
                else:
                ...
                # ✨ 그래프 배경색을 완전 투명(rgba(0,0,0,0))으로 처리하여 회색 상자 전면 제거
                ...
                # 🔴 가로축/세로축 글자 크기 대폭 확대 (size=18) 및 진한 고딕 스타일 고정
                if is_mobile_view:
