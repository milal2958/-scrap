            ...
            if st.form_submit_button("로그인"):
                ...
                if sheet_client:
                    try:
                        ...
                        USER_CREDENTIALS = {str(row['사번']): str(row['비밀번호']) for row in user_records}
...
if login():
    ...
    # 🛠️ [신규 기능] 사이드바 내 비밀번호 자체 변경 메뉴 이식
                ...
                if not current_pw or not new_pw or not confirm_pw:
                    ...
                    if client:
                        try:
                            ...
                            # 시트 전체 명단 로드 (행 번호 추적을 위해 enumerate 활용)
                            found_row_idx = None
                            is_current_pw_correct = False
                            
                            for idx, row in enumerate(user_records, start=2): # 헤더 다음줄인 2번째 행부터 시작
                                if str(row['사번']) == str(st.session_state['user_id']):
                                    found_row_idx = idx
                                    if str(row['비밀번호']) == hash_password(current_pw):
                            ...
                            if not is_current_pw_correct:
                                ...
                                # 암호화된 새 패스워드 해당 행의 B열(비밀번호)에 정밀 업데이트
    ...
    with tab2:
        ...
        if not df.empty:
            st.markdown("#### 📊 실시간 조별 스케줄 및 처리 현황 요약")
            ...
            if '요일' in filtered_df.columns: filtered_df = filtered_df.drop(columns=['요일'])
                ...
                if st.button("💾 변경사항 구글 시트에 저장하기"):
                    ...
                    if client:
                        ...
                        with st.spinner("데이터 업데이트 중..."):
                            ...
                            if '요일' not in save_df.columns: save_df.insert(1, '요일', "")
    ...
    with tab3:
        ...
        if not df_analysis.empty:
            ...
            if not df_trend_filtered.empty:
                ...
                fig_monthly.update_layout(plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', margin=dict(t=30, b=30, l=10, r=140), font=dict(family="Pretendard, sans-serif", size=14), hovermode="closest", legend=dict(orientation="h", yanchor="top", y=-0.18, xanchor="center", x=0.5, title_text=""))
            ...
            if not df_cause_filtered.empty:
                ...
                fig_cause.update_layout(height=380, bargap=0.15, plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', margin=dict(t=20, b=20, l=10, r=140), font=dict(family="Pretendard, sans-serif", size=14))
            ...
            if not df_mac_filtered.empty:
                ...
                st.warning("⚠️ 설비 데이터가 없습니다.")

            st.markdown("---")

            # --- 4. 처리 판단별 ---
            st.markdown("### 📋 4. 처리 판단별 스크랩 현황 분석")
            start_date_status = st.date_input("처리판단 시작일", datetime.now().replace(day=1), key="status_start")
            end_date_status = st.date_input("처리판단 종료일", datetime.now(), key="status_end")

            mask_status = (df_analysis['날짜'] >= start_date_status) & (df_analysis['날짜'] <= end_date_status)
            df_status_filtered = df_analysis.loc[mask_status].copy()

            if not df_status_filtered.empty:
                status_summary = df_status_filtered.groupby('처리 판단')['발생량_ton'].sum().reset_index()
                ...
                max_val_status = status_summary['발생량_ton'].max()
                fig_status.update_layout(height=350, bargap=0.15, plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', margin=dict(t=20, b=20, l=10, r=140), font=dict(family="Pretendard, sans-serif", size=14))
