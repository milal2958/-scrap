...
# --- 1. 페이지 설정 및 디자인 ---
st.set_page_config(page_title="정련Sub팀 스크랩 관리 시스템 v4.3", page_icon="⚙️", layout="wide")
    ...
    with tab2:
        ...
        with st.container():
            ...
            dispose_type = st.radio(
                "폐기 분류 유형 선택", 
                ["스크랩(고무)", "스크랩(마대)", "카본", "약품", "분진"], 
                horizontal=True, 
                key="dispose_type_input"
            )
            ...
            # 🌟 스크랩(고무) 선택 시 하부 UI 동적 오픈 및 순서 제어
            if dispose_type == "스크랩(고무)":
                ...
                if is_history_unknown:
                    ...
                    with rc1: 
                        rubber_p_date = str(st.date_input("발생 생산일 선택", datetime.now(), key="rubber_date_input"))
                    with rc2: 
                        rubber_shift = st.selectbox("발생 생산조 선택", ["A조", "B조", "C조", "D조", "확인불가"], key="rubber_shift_input")
                    with rc3:
            ...
            st.write("**실제 처리 중량 입력**")
            dispose_weight = st.number_input("폐기 및 처리 중량 (kg)", min_value=0.0, step=0.5, key="dispose_weight_input")
            
            st.markdown("---")
            st.write("**기타 참고사항**")
            dispose_memo = st.text_input(
                "비고 사항 입력", 
                placeholder="(고무정보 등 모든내용 기록)", 
                key="dispose_memo_input"
            )
            ...
            if st.button("💾 처리 실적 저장하기", key="btn_dispose_submit"):
                ...
                if dispose_type == "스크랩(고무)" and not is_history_unknown and len(r_comp_text) < 3:
                    st.error("⚠️ Comp'd명 3자리를 정확히 입력하거나 '전체 이력 확인불가'를 체크해주세요.")
                    ...
                    if client:
                        try:
                            ...
                            proc_data = [
                                ...
                                dispose_weight, extended_memo, 
                                datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 
                                st.session_state["user_id"]
                            ]
                            proc_sheet.append_row(proc_data)
                            
                            st.session_state["disp_success_msg"] = f"✅ [{dispose_shift}] 실제 처리 중량 {dispose_weight:,} kg 기록이 안전하게 저장되었습니다!"
                            st.cache_resource.clear()
                            if "raw_df" in st.session_state: 
                                del st.session_state["raw_df"]
                            st.rerun()
    ...
    with tab3:
        ...
        if not df.empty:
            ...
            try:
                ...
                if st.button("💾 변경사항 구글 시트에 저장하기", key="btn_save_grid_changes"):
                    client = connect_google_sheet_client()
                    if client:
                        sheet = client.open("현장스크랩데이터").sheet1
                        ...
                        if not edited_rows:
                            ...
                            with st.spinner("수정된 셀만 안전하게 동기화 중..."):
                                headers = [str(c) for c in df.columns if c != '__sheet_row_idx']
                                col_map = {name: i+1 for i, name in enumerate(headers)}
                                batch_cells = []
                                
                                for numeric_idx, changes in edited_rows.items():
                                    actual_row = filtered_df.iloc[int(numeric_idx)]
                                ...
                                if batch_cells:
                                    ...
                                    st.toast("🎉 변경 내역이 완벽하게 반영되었습니다!", icon="💾")
                                    st.cache_resource.clear()
                                    st.session_state["raw_df"] = get_data()
                                    time.sleep(0.5)
                                    st.rerun()
        ...
        if not df_analysis.empty:
            ...
            if not df_filtered.empty:
                ...
                if target_mac_col:
                    ...
                    st.warning("⚠️ 시트 내에 '발생 호기' 또는 '발생 설비' 컬럼이 존재하지 않아 그래프를 그릴 수 없습니다.")
