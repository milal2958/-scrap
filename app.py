...
st.markdown("""
    ...
    /* 📱 모바일 환경 최적화 레이아웃 (겹침 방지 및 카드 가시성 강화) */
    @media (max-width: 768px) {
        .block-container {
            padding-left: 0.5rem !important; 
            padding-right: 0.5rem !important;
            padding-top: 3.5rem !important; /* 상단 타이틀 및 사이드바 아이콘과의 겹침 방지 */
        ...
        /* 스마트폰에서 카드가 안 보이는 버그 수정: 다크모드 대응 및 테두리 명확화 */
        .metric-card {
    ...
    with tab3:
        ...
        if not df_analysis.empty:
            ...
            if not df_filtered.empty:
                ...
                monthly_summary = df_filtered.groupby(['년월', '生産 조'])['발생량_ton'].sum().reset_index()
                ...
                # 🛠️ 요청사항 반영: 가로형인 모바일 뷰일 때 왼쪽 여백(l=0) 설정으로 왼쪽 밀착
                ...
                # 🛠️ 요청사항 반영: l=0으로 주어 완전히 왼쪽 정렬로 바짝 당김
