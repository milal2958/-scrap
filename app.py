            ...
            if st.button("🚀 데이터 등록하기"):
                if len(comp_text) < 1 or reg_amt <= 0:
                    ...
                    if sheet:
                        ...
                        # 🛠️ [패치 1] 알림 가시성을 위해 눈에 띄는 상단 알림(toast) 창으로 변경
                        st.toast(f"✅ 구글 시트에 스크랩 정보가 정상 등록되었습니다!", icon="🚀")
    ...
    with tab3:
        ...
        if not df_analysis.empty:
            ...
            if not df_trend_filtered.empty:
                ...
                # 🛠️ [패치 3-1] 트렌드 그래프 우측 숫자 짤림 방지 마진 확보 (r=100)
                max_val_trend = monthly_summary['발생량_kg'].max()
                ...
                if is_mobile_view:
                    fig_monthly.update_xaxes(title_text="", showgrid=False, showticklabels=False, ticks="", zeroline=False, range=[0, max_val_trend * 1.25])
                    ...
                    fig_monthly.update_yaxes(title_text="", showgrid=False, showticklabels=False, ticks="", zeroline=False, range=[0, max_val_trend * 1.25])
            ...
            if not df_cause_filtered.empty:
                cause_summary = df_cause_filtered.groupby('추정 원인')['발생량_kg'].sum().reset_index()
                cause_summary = cause_summary.sort_values(by='발생량_kg', ascending=True)
                ...
                fig_cause = px.bar(
                    ...
                    orientation='h',
                    text_auto='.1f',
                    labels={'발생량_kg': '발생량 (kg)', '추정 원인': '추정 원인'}
                )
                ...
                # 🛠️ [패치 2 & 3-2] 막대가 안 벌어지고 촘촘하게 붙도록 height 제어 / 숫자 안 짤리도록 마진 확대 및 자동 가로 배율 증가
                max_val_cause = cause_summary['발생량_kg'].max()
                fig_cause.update_layout(
                    height=380, # 차트 프레임 높이를 최적화하여 컴팩트하게 밀착
                    bargap=0.15, # 막대들 사이의 간격을 최소화하여 촘촘하게 조정
                    plot_bgcolor='rgba(0,0,0,0)',
                    paper_bgcolor='rgba(0,0,0,0)',
                    margin=dict(t=20, b=20, l=10, r=110), # 우측 여백을 110px 확보하여 대용량 숫자 짤림 절대 방지
                    font=dict(family="Pretendard, Malgun Gothic, sans-serif", size=14),
                    hovermode="closest"
                )

                # 최우측 숫자 표기가 프레임 밖으로 넘어가지 않도록 한계 값 20% 여유 설정
