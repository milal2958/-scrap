...
if login():
    ...
    with tab1:
        ...
        with st.container():
            ...
            # 버튼 누른 후 결과를 고정해서 보여주기 위한 비어있는 컨테이너 선언
            ...
            if st.button("🚀 데이터 등록하기"):
                if len(comp_text) < 1 or reg_amt <= 0:
                    ...
                    if sheet:
                        ...
                        # 🛠️ [패치] 버튼 바로 아래(컨테이너 내부)에 녹색 성공 알림 고정 출력
                        msg_container.success(f"✅ 구글 시트에 스크랩 발생 정보가 정상 등록되었습니다!")
                        st.toast(f"✅ 등록 완료!", icon="🚀")
                        
                        st.cache_resource.clear()
                        if "raw_df" in st.session_state: del st.session_state["raw_df"]
                        # 등록 결과를 현장 작업자가 직접 확인한 뒤 새로고침 되도록 rerun() 타이밍을 조절하거나 주석처리하여 유지할 수 있습니다.
