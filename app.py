...
if login():
    ...
    with tab2:
        ...
        with st.container():
            ...
            # 마대 / 파레트 구분 선택
            st.write("**적재 형태 구분**")
            load_type = st.radio("용기/품목 형태 구분", ["마대", "파레트(Pallet)"], horizontal=True, key="load_type_input")
            ...
            st.markdown("---")
            st.write("**기타 참고사항 및 처리 정보**")
            dispose_memo = st.text_input("비고 (예: 폐기 업체명 혹은 작업자 특이사항 등)", placeholder="필요시 기입하세요.", key="dispose_memo_input")
            ...
            # 💡 구글 시트에 "스크랩처리내역" 같은 전용 워크시트가 있다고 가정하거나 명시적 분기가 요구되는 지점입니다.
            if st.button("💾 처리 실적 저장하기", key="btn_dispose_submit"):
                if dispose_weight <= 0:
                    ...
                    if client:
                        try:
                            # 현장스크랩데이터 파일 내에 별도 탭인 '스크랩처리내역' 시트가 존재해야 정상 작동합니다.
                            # 만약 존재하지 않는다면 기본 sheet1 하단에 쌓거나 연동 구조를 맞추어야 합니다.
                            db_file = client.open("현장스크랩데이터")
                            try:
                                proc_sheet = db_file.worksheet("스크랩처리내역")
                            except Exception:
                                # 워크시트가 없으면 사용자 편의를 위해 첫 시트에 임시 기입하거나 에러 메시지 유도
                            ...
                            st.session_state["disp_success_msg"] = f"✅ [{load_type}] 실제 처리 중량 {dispose_weight:,} kg 기록이 데이터베이스에 정상 연동되었습니다!"
                            st.cache_resource.clear()
                            if "raw_df" in st.session_state: 
                                del st.session_state["raw_df"]
                            st.rerun()
