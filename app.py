...
st.markdown("""
    ...
    /* 📱 모바일 환경 최적화 레이아웃 (겹침 방지 튜닝) */
    @media (max-width: 768px) {
        .block-container {
            padding-left: 0.7rem !important; 
            padding-right: 0.7rem !important;
            padding-top: 3.5rem !important; /* 🌟 상단 타이틀 및 사이드바 아이콘과의 겹침 방지를 위해 마진 확대 */
...
if login():
                ...
                if st.button("💾 변경사항 구글 시트에 저장하기"):
                    ...
                    if client:
                        ...
                        with st.spinner("데이터 업데이트 중..."):
                            ...
                            sheet.update_values(f'A2:O{len(update_vals)+1}', update_vals)
    ...
    with tab3:
        ...
        if not df_analysis.empty:
            ...
            with ana_col1:
                start_date = st.date_input("분석 시작일", df_analysis['날짜'].min() if not df_analysis['날짜'].isna().all() else datetime.now().replace(day=1), key="ana_start")
            with ana_col2:
