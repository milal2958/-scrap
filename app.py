...
st.markdown("""
    ...
    @media (max-width: 768px) {
        /* 시스템 기본 상단 여백 최소화 */
        .block-container {
            padding-left: 0.5rem !important; /* 여백을 더 줄여 그래프 크기 확보 */
        ...
        if not df_analysis.empty:
            ...
            if not df_trend_filtered.empty:
                ...
                if is_mobile_view:
                    ...
                    # 🔴 수치 텍스트를 빨간색(red), 굵게(bold), 크게(size=15) 지정하고 바 바깥쪽 배치
                    fig_monthly.update_traces(
                        textposition='outside',
                        textfont=dict(color='red', size=15, family='Pretendard', weight='bold')
                    )
                else:
                ...
                fig_monthly.update_layout(
                    ...
                    margin=dict(t=20, b=30, l=10, r=40), # 글자가 잘리지 않도록 우측 마진 조정
                    legend=dict(
                        orientation="h", 
                        yanchor="top", 
                        y=-0.15, 
                ...
                # 🛠️ [핵심 패치] 가로축의 숫자, 눈금선, 격자 라인까지 완전 제거하여 그래프 폭 확장
                if is_mobile_view:
                    fig_monthly.update_xaxes(
                        title_text="", 
                        showgrid=False,      # 격자선 제거
                        showticklabels=False, # 하단 축 숫자 제거
                        ticks="",            # 눈금 표시 제거
                        zeroline=False
                    )
