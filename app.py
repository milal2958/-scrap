...
# --- 1. 페이지 설정 ---
st.set_page_config(page_title="정련Sub팀 스크랩 관리 시스템 v3.1", page_icon="⚙️", layout="wide")

USER_CREDENTIALS = {"21200364": "enslfkd11!", "user1": "5678", "user2": "9999"}
...
def login():
    ...
    if not st.session_state["logged_in"]:
        st.subheader("⚙️ 정련Sub팀 스크랩 시스템 로그인")
    ...
    with tab3:
        ...
        if not df_analysis.empty:
            ...
            if not df_filtered.empty:
                ...
                # 1. 조별 발생량 막대 그래프
                st.write("👥 **생산 조별 총 발생량 현황 (kg 환산)**")
                st.info("💡 **환산 기준**: 'batch' 단위는 1 batch당 **200kg**으로 합산되었습니다.")
                    ...
                    for idx, row in uncomp_summary.iterrows():
                        with uncomp_cols[idx]:
                            st.metric(label=f"[{row['생산 조']}] 미완료", 
                                      ...
                                      delta_color="inverse") # 중량은 많을수록 경고(빨강)
                else:
                    st.success("✅ 선택 기간 내 모든 항목의 처리가 완료되었습니다!")

                st.markdown("---")

                # 3. 호기별 및 원인별 상세 분석
                st.write("⚙️ **호기별 스크랩 발생 상세 (kg 환산)**")
                mac_chart_data = df_filtered.groupby('발생 호기')['발생량_kg'].sum().reset_index()
                fig_mac = px.bar(mac_chart_data, x='발생 호기', y='발생량_kg', 
                                 text_auto='.1f', color='발생량_kg', 
                                 color_continuous_scale='Reds',
                                 category_orders={"발생 호기": MAC_LIST})
                st.plotly_chart(fig_mac, use_container_width=True)

                st.write("🔍 **추정 원인별 발생 분포**")
                cause_chart_data = df_filtered.groupby('추정 원인').size().reset_index(name='건수')
                fig_cause = px.pie(cause_chart_data, values='건수', names='추정 원인', hole=0.4)
                st.plotly_chart(fig_cause, use_container_width=True)

            else:
                st.warning("선택한 기간 내에 데이터가 없습니다.")
        else:
