...
if login():
    ...
    # --- [TAB 1] 데이터 등록 ---
    with tab1:
        st.subheader("📍 보류고무 발생 정보 입력")  # 💡 이름 변경
        with st.container():
            # [1단계] 기본 정보 입력 (날짜, 조, 설비)
            row1_col1, row1_col2, row1_col3 = st.columns(3)
            with row1_col1: reg_date = st.date_input("생산 일자", datetime.now())
            with row1_col2: reg_shift = st.selectbox("생산 조", ["A조", "B조", "C조", "D조"])
            with row1_col3: reg_mac = st.selectbox("발생 호기(설비)", MAC_LIST)
            
            st.markdown("---")
            
            # [2단계] 보류고무 규격(Comp'd명) 정보 입력
            st.write("**Comp'd명 입력**")
            ...
            # [3단계] 발생 원인 섹션 (추정 원인, 처리 판단, 상세 설명 통합)
            st.write("**발생 원인**")  # 💡 타이틀 명칭 일원화
            ...
            # [5단계] 보관 위치 섹션
            st.write("**보관 위치**")
            reg_loc = st.text_input("현재 보관 위치 입력", placeholder="예: 정련 보관소 A-1")
    ...
    with tab2:
        ...
        if not df.empty:
            ...
            with f_col2:
                selected_macs = st.multiselect("발생 호기 필터", options=["전체"] + mac_options, default=["전체"])
        ...
        if not df_analysis.empty:
            ...
            if not df_filtered.empty:
                ...
                if '년월' in df_filtered.columns and '생산 조' in df_filtered.columns:
                    ...
                    if is_mobile_view:
                        fig_monthly = px.bar(monthly_summary, x='발gen량_ton', y='생산 조', color='년월', barmode='group', text='레이블', orientation='h', labels={'발생량_ton': '발생량 (ton)'}, color_discrete_sequence=px.colors.qualitative.Set2)
                    else:
