...
def connect_google_sheet():
    ...
    try:
        # Streamlit Cloud의 Secrets 설정이 되어 있는지 확인
        if "gcp_service_account" in st.secrets:
            creds_info = st.secrets["gcp_service_account"]
            creds = Credentials.from_service_account_info(creds_info, scopes=scopes)
        else:
            # 로컬 테스트용 (내 컴퓨터에 secrets.json이 있을 때)
...
if login():
    ...
    with tab2:
        ...
        if not df.empty:
            ...
            try:
                ...
                # '단위' 컬럼을 찾아 그 다음 위치 인덱스 계산 (없으면 발생량 뒤로, 둘 다 없으면 맨 앞으로)
                if "단위" in current_cols:
                    insert_idx = current_cols.index("단위") + 1
                elif "발생량" in current_cols:
                    insert_idx = current_cols.index("발생량") + 1
                else:
                    insert_idx = 0
                
                # 계산된 위치에 타겟 컬럼 삽입 (원하는 순서를 유지하기 위해 역순으로 삽입)
                for col in reversed(target_cols):
                    if col in filtered_df.columns:
                        current_cols.insert(insert_idx, col)
                # ==========================================
                
                edited_df = st.data_editor(
                    filtered_df,
                    column_order=current_cols, # 👈 재배치된 컬럼 순서 적용
                    column_config={
    ...
    with tab3:
        ...
        if not df_analysis.empty:
            ...
            with a_col2:
                end_date = st.date_input("분석 일", datetime.now())
