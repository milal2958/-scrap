...
st.markdown("""
    ...
    .mobile-card h5 { margin: 0 0 8px 0; color: #333333; font-size: 14px; }
    .mobile-card .val { font-size: 20px; font-weight: bold; color: #111111; }
    .mobile-card .delta { font-size: 12px; color: #ff4b4b; margin-top: 4px; }
    </style>
...
if login():
    ...
    with tab3:
        ...
        if not df_analysis.empty:
            ...
            # 👇 [개선 1] 분석 전체 제어 기간 설정을 가장 위로 이동 (가독성 향상)
            st.markdown("#### 🔍 분석 조회 기간 설정")
            date_col1, date_col2 = st.columns(2)
            with date_col1:
                start_date = st.date_input("조회 시작일", datetime.now().replace(day=1))
            with date_col2:
            ...
            # --- [그래프 1: 조별/월별 발생 추이] ---
            # 사용자가 설정한 기간(df_filtered)을 기준으로 그래프가 연동되도록 변경했습니다.
            if not df_filtered.empty:
                st.markdown(f"#### 📅 조별 및 월별 발생 트렌드 ({start_date} ~ {end_date})")
                monthly_summary = df_filtered.groupby(['년월', '생산 조'])['발생량_kg'].sum().reset_index()
                ...
                st.warning("⚠️ 선택한 기간에 해당하는 발생 데이터가 없습니다.")

            # 👇 [개선 2] 모바일 화면(스마트폰) 대응 미완료 잔여 집계 레이아웃 개선
            st.markdown("#### ⚠️ 생산 조별 미완료(잔여) 항목 집계")
            
            # 선택된 기간 내의 미완료 데이터를 추출하되, 직관성을 위해 가변형 그리드로 디자인
            uncompleted_df = df_filtered[df_filtered['처리 완료'] == False].copy() if not df_filtered.empty else pd.DataFrame()
            ...
            if not uncompleted_df.empty:
                ...
                # HTML과 CSS Grid를 사용하여 스마트폰에서는 세로로 자동 정렬되고, PC에서는 가로로 펼쳐지도록 패치
                card_html = '<div class="mobile-card-container">'
                for idx, row in uncomp_summary.iterrows():
                    card_html += f"""
                    <div class="mobile-card">
                        <h5>{row['생산 조']} 미완료</h5>
                        <div class="val">{row['미완료건수']} 건</div>
                        <div class="delta">+{row['미완료중량_kg']:,.1f} kg</div>
                    </div>
                    """
                card_html += '</div>'
                st.markdown(card_html, unsafe_allow_html=True)
            else:
                st.success("✅ 선택한 기간 내 모든 스크랩 처리가 완료되었습니다!")

            st.markdown("---")

            # --- [섹션: 요약 데이터 지표 및 원인별 분석] ---
            if not df_filtered.empty:
                col_b1, col_b2 = st.columns([1, 1])
                
                with col_b1:
                    st.markdown("#### 🏆 선택 기간 핵심 지표 요약")
                    total_kg = df_filtered['발생량_kg'].sum()
                    
                    # 기존 좁게 터지던 모바일 화면 구조를 컨테이너형 단독 메트릭으로 시원하게 분할
                    st.metric("선택 기간 총 발생량", f"{total_kg:,.1f} kg")
                    
                    uncomp_filtered = df_filtered[df_filtered['처리 완료'] == False]
                    st.metric("선택 기간 내 미완료 건수", f"{len(uncomp_filtered)} 건", delta=f"{uncomp_filtered['발생량_kg'].sum():,.1f} kg", delta_color="inverse")

                with col_b2:
                    st.markdown("#### ⚠️ 원인별 스크랩 발생 비중")
                    ...
                    fig_pie.update_layout(margin=dict(t=30, b=10, l=10, r=10), legend=dict(orientation="h"))
