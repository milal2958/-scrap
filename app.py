...
if login():
    ...
    with tab2:
        ...
        with st.container():
            ...
            # 기타 참고사항 (안내 '예' 삭제)
            st.write("**기타 참고사항**")
            dispose_memo = st.text_input("처리 정보 기입 (폐기 업체명 혹은 특이사항 등)", placeholder="참고사항을 입력하세요.", key="dispose_memo_input")
                ...
                if dispose_weight <= 0:
                    ...
                    if client:
                        try:
                            ...
                            st.session_state["disp_success_msg"] = f"✅ [{dispose_shift} / {dispose_type}] 실제 처리 중량 {dispose_weight:,} kg 기록이 정상 연동되었습니다!"
                            st.cache_resource.clear()
                            if "raw_df" in st.session_state: 
                                del st.session_state["raw_df"]
                            st.rerun()
