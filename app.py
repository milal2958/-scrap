        ...
        if not df.empty:
            ...
            for i, shift_name in enumerate(shifts_list):
                with stat_cols[i]:
                    ...
                    done_weight = df_done['__wt_ton'].sum()
                    
                    # 2. 처리 예정일 기록 건수 및 중량 집계 (완료되지 않았고 예정일이 있는 항목)
                    df_plan = df_shift_stat[(df_shift_stat['처리 완료'] == False) & (df_shift_stat['처리 예정일'].notna())]
                    plan_count = len(df_plan)
                    plan_weight = df_plan['__wt_ton'].sum()
                    
                    # 3. 미입력 건수 및 중량 집계 (완료되지 않았고 예정일도 없는 항목)
                    df_missing = df_shift_stat[(df_shift_stat['처리 완료'] == False) & (df_shift_stat['처리 예정일'].isna())]
                    missing_count = len(df_missing)
                    missing_weight = df_missing['__wt_ton'].sum()
                    
                    # UI 익스팬더 박스 형태로 가독성 높게 노출
        ...
        if not df_analysis.empty:
            ...
            if not df_trend_filtered.empty:
                ...
                if is_mobile_view:
                    ...
                    fig_monthly = px.bar(
                        ...
                        barmode='group', 
                        text='레이블', 
                        labels={'발sub량_ton': '발생량 (ton)', '년월': '월별'},
                        color_discrete_sequence=px.colors.qualitative.Set2
                    )
                ...
                fig_monthly.update_layout(
                    ...
                    margin=dict(t=30, b=30, l=10, r=140), 
                    legend=dict(
                        orientation="h", 
                        yanchor="top", 
                        y=-0.18, 
            ...
            if not df_cause_filtered.empty:
                ...
                fig_cause.update_layout(
                    ...
                    plot_bgcolor='rgba(0,0,0,0)',
                    paper_bgcolor='rgba(0,0,0,0)',
                    margin=dict(t=20, b=20, l=10, r=140), 
            ...
            if not df_mac_filtered.empty:
                ...
                fig_mac.update_traces(
                    marker_color='#7ED321', 
                    textposition='outside',
                    textfont=dict(color='#0052CC', size=16, family='Pretendard, Malgun Gothic, sans-serif', weight='bold')
                )
                ...
                fig_mac.update_xaxes(
                    showgrid=False, 
                    zeroline=False,
                    tickfont=dict(size=16, family='Pretendard, Malgun Gothic, sans-serif', weight='bold')
                )
            ...
            # ==========================================
            # 4️⃣ [처리 판단별 발생량 분석 - 가로형 막대]
            # ==========================================
            st.markdown("### 📋 4. 처리 판단별 스크랩 현황 분석")
            
            date_col7, date_col8 = st.columns(2)
            with date_col7:
            ...
            if not df_status_filtered.empty:
                ...
                fig_status = px.bar(
                    ...
                    orientation='h',
                    text='레이블',
                    labels={'발생량_ton': '발생량 (ton)', '처리 판단': '처리 판단'}
                )
                ...
                st.warning("⚠️ 선택한 기간에 해당하는 처리 판단 데이터가 없습니다.")
