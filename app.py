...
# 모바일 화면 최적화를 위한 커스텀 패딩 및 스타일 제어
st.markdown("""
    <style>
    .main { background-color: #f8f9fa; }
    div[data-testid="stExpander"] { border: none !important; box-shadow: none !important; }
    
    /* 모바일 환경에서 좌우 여백을 줄여 차트 영역을 최대한 확보 */
    ...
    with tab3:
        ...
        if not df_analysis.empty:
            ...
            if not df_trend_filtered.empty:
                ...
                # 💡 [모바일 최적화의 핵심] 대략 일자 간격이나 데이터 개수를 체크하여 모바일 여부 임베딩 파악
                # 모바일 세로 화면 기준(날짜 범위가 좁거나 스크린이 작을 때) 가로 막대 그래프가 압도적으로 편합니다.
                is_mobile_view = len(monthly_summary['년월'].unique()) >= 3 or len(monthly_summary) >= 8

                if is_mobile_view:
                    # 📱 모바일 버전: 가로형(Horizontal) 바 차트 (글자가 절대 깨지거나 뭉개지지 않음)
                    fig_monthly = px.bar(
                        monthly_summary, 
                        x='발생량_kg', 
                        y='생산 조', 
                ...
                # ✨ 공통 UI/UX 프리미엄 디자인 고도화
                fig_monthly.update_layout(
                    plot_bgcolor='rgba(250,250,250,0.8)', # 차트 배경 그리드 부드럽게 완화
                    paper_bgcolor='rgba(0,0,0,0)',
                    margin=dict(t=40, b=30, l=20, r=20),
                    # 범례(Legend)를 차트 최하단 가로 정렬하여 모바일 화면에서 차트가 가려지는 문제 완전 해결
                    legend=dict(
                        orientation="h", 
                        yanchor="top", 
                        y=-0.15, 
                        xanchor="center", 
                        x=0.5,
                        title_text="" # 불필요한 '년월' 가이드 타이틀 제거로 공간확보
                    ),
                    font=dict(family="Pretendard, sans-serif", size=12),
                    hovermode="unified",
                    bargap=0.2, # 막대 간격 밸런스 조정
                    bargroupgap=0.08
                )
