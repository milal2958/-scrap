...
if login():
    ...
    with tab1:
        ...
        with st.container():
            ...
            # 🛠️ 버튼 바로 아래에 상시 알림을 배치하기 위한 전용 컨테이너 고정
            alert_container = st.container()

            # 유지형 알림 렌더링 로직 (새로고침 후에도 유지됨)
            ...
            if st.button("🚀 데이터 등록하기"):
                ...
                if len(comp_text) < 1 or reg_amt <= 0:
                    ...
                    if client:
                        ...
                        # 🛠️ 현재 제출 내역을 세션에 각인시켜 연속 클릭 방지
                        st.session_state["last_submitted"] = current_submission
                        
                        # 🛠️ 성공 메시지 박스 데이터 세션에 박제 (유지 기능)
