                ...
                if st.button("💾 변경사항 구글 시트에 저장하기"):
                    ...
                    if sheet:
                        with st.spinner("구글 시트 업데이트 중..."):
                            ...
                            st.cache_resource.clear()
                            st.session_state["raw_df"] = get_data()
                            st.success("✅ 필터 상태와 무관하게 전체 데이터가 안전하게 업데이트되었습니다!")
                            st.rerun()
            except Exception as e:
    ...
    with tab3:
        ...
        if not df_analysis.empty:
            ...
            if not df_trend_filtered.empty:
                ...
                if is_mobile_view:
                    fig_monthly.update_xaxes(title_text="", showgrid=False, showticklabels=False, ticks="", zeroline=False)
                    ...
                    fig_monthly.update_yaxes(title_text="", showgrid=False, showticklabels=False, ticks="", zeroline=False)
            ...
            # ==========================================
            # 2️⃣ [신규 섹션 2: 추정 원인별 발생량 분석]
            # ==========================================
            st.markdown("### 🔍 2. 추정 원인별 스크랩 발생량 현황")
            
            # 별도로 날짜 설정을 할 수 있도록 다른 키(key)의 달력 배치
            ...
            if not df_cause_filtered.empty:
                ...
                # 🛠️ 높은 순(내림차순) 정렬 처리
                cause_summary = cause_summary.sort_values(by='발생량_kg', ascending=True) # Plotly 가로 차트는 아래에서 위로 채워지므로 데이터 정렬을 오름차순으로 해두어야 시각적으로 맨 위가 최고값이 됩니다.
                ...
                # 🔵 파란색 수치 표시 및 18pt 굵은 고딕체 패치
                fig_cause.update_traces(
                    marker_color='#4A90E2', # 원인 차트 전용 시원한 스카이블루 계열
                    textposition='outside',
                    textfont=dict(color='#0052CC', size=18, family='Pretendard, Malgun Gothic, sans-serif', weight='bold')
                )
                ...
                # 원인 항목(좌측 레이블) 글씨 크게 확대 및 가로 격자선 삭제
                fig_cause.update_xaxes(title_text="", showgrid=False, showticklabels=False, ticks="", zeroline=False)
